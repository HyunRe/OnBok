...
public class CsvFileReaderService {
    ...
    private final Optional<BookEsService> bookEsService;

    public CsvFileReaderService(
            ResourceLoader resourceLoader,
            BookCommandService bookCommandService,
            @Autowired(required = false) BookEsService bookEsService
    ) {
        this.resourceLoader = resourceLoader;
        this.bookCommandService = bookCommandService;
        this.bookEsService = Optional.ofNullable(bookEsService);
    }
    ...
    public void csvFileToElasticSearch() {
        if (bookEsService.isEmpty()) {
            System.out.println("ElasticSearch is disabled. Skipping ElasticSearch indexing.");
            return;
        }
            ...
            try (Reader reader = new InputStreamReader(resource.getInputStream(), StandardCharsets.UTF_8);
                ...
                for (CSVRecord record: csvParser) {
                    ...
                    BookEs bookEs = BookEs.builder()
                            .title(title).author(author).company(company).price(price).imageUrl(imageUrl).summary(summary)
                            .build();
                    bookEsService.get().insertBookEs(bookEs);
